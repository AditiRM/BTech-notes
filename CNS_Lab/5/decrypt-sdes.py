#Tables requires for respective permutations
...
def decrypt(cipherText, K1, K2):
    cipherText = [int(x) for x in str(cipherText)]
    IP8 = perm(cipherText, IPtable)       #Inittial permutation on plaintext
    file2.write("\nIP Output: " + printlist(IP8))
    Mid1 = fk(IP8, K2)
    file2.write("\nfirst Fk output: " + printlist(Mid1))
    ...
    Mid2 = fk(swap, K1)
    file2.write("\nSecond Fk output: " + printlist(Mid2))
    #Apply IP inverse 
    CipherText = perm(Mid2, IPInvtable)
    return CipherText

file = open("ip2.txt", 'r')
file2 = open("op2.txt","w+")
input = file.read()
input = input.split("\n")
key = input[0]
CipherText = input[1]
...
K1, K2 = Keygen(key)
file2.write("K1: " + printlist(K1))
file2.write("\nK2: " + printlist(K2))
CipherText = decrypt(CipherText, K1, K2)
print("Plain Text is generated in file op2.txt")
file2.write("\nPlain Text: " + printlist(CipherText))
